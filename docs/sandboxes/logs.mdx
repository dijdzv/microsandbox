...
## Local policy-deny events (Orbit fork)

The Orbit fork can write network-policy denials as JSON Lines when the sandbox
runtime receives `MSB_DENY_LOG_PATH`. Set a distinct path for each sandbox; the
file is created with mode `0600` on Unix. If the path cannot be opened, startup
fails instead of running with policy-deny observation silently disabled. The
record contains `sandbox_id`, transport, destination host/IP and port, policy
origin (`tenant` or `platform`), and a short reason. It does not include
request bodies or secret values. Denied external ICMP Echo Requests are recorded
even when the host cannot open an unprivileged ping socket; an allowed Echo
Request still requires that host capability to be relayed. This is an optional
local diagnostic stream, not part of `msb logs` or an upstream microsandbox
event API.
