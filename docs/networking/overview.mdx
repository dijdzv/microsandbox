...
The VM's private IP and MAC address are leased while it runs. They do not
identify a sandbox permanently: after a stop and restart, it may receive a
different address. Use published ports or the supported host connection rather
than saving the guest address for later runs.
